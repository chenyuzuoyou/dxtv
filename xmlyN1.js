/*!
 * @name NxmlyfmN1
 * @description xmlyfmN3.4 喜马拉雅FM 终极修复版 V2 彻底攻克专辑列表空、接口风控限流
 * @version v1.7.5 FULLFIX
 * @author codex
 * @key csp_xmlyfm
 */
...
async function loadRecommendedAlbums(page = 1) {
  const urls = [
    ...
    `https://www.ximalaya.com/revision/search?core=album&kw=${encodeURIComponent('热门')}&page=${page}&rows=${PAGE_LIMIT}&spellchecker=true&condition=relation&device=web`
  ]
  for (const url of urls) {
    try {
      const data = await fetchJson(url, { 'User-Agent': UA_APP })
      const list = firstArray(data?.data?.list, data?.data?.albums, data?.data?.result?.response?.docs)
...
// 重构的核心逻辑：深度攻克专辑列表空白问题
async function loadAlbumTracks(albumId, page = 1) {
  let allTracks = [];
  let currentPage = page || 1; 
  const randomDeviceId = '88' + Math.random().toString().substring(2, 16);

  // 接口优先级重构：使用小程序与轻量移动网关接口，绕过网页防爬
  const apiList = [
    // 1. 小程序/轻量网关接口（带匿名鉴权参数，目前最稳）
    {
      url: `https://mobile.ximalaya.com/mobile/v1/album/track/ts-${Date.now()}?albumId=${albumId}&device=android&pageId=${currentPage}&pageSize=100&isAsc=true&deviceId=${randomDeviceId}&version=7.3.0`,
      ua: UA_APP,
      referer: 'https://servicewechat.com/wx70eeac35bc23a1a4/devtools/page-frame.html'
    },
    // 2. 备用移动端无拦截路径
    {
      url: `https://mobile.ximalaya.com/mobile/v1/album/track?albumId=${albumId}&device=android&pageId=${currentPage}&pageSize=100&isAsc=true`,
      ua: UA_MOBILE,
      referer: `https://m.ximalaya.com/album/${albumId}`
    },
    // 3. 通用H5公开网关
    {
      url: `https://m.ximalaya.com/m-revision/common/album/queryAlbumTrackRecordsPage?albumId=${albumId}&pageId=${currentPage}&pageSize=100`,
      ua: UA_MOBILE,
      referer: `https://m.ximalaya.com/album/${albumId}`
    },
    // 4. PC改版多重兜底
