/*!
 * @name a30xmlyfmN
 * @description xmlyfm3.4 喜马拉雅FM 终极修复版 解决大量专辑空白、接口403限流及iOS/Listenify卡片点击空白问题
 * @version v1.6.7 FULLFIX
 * @author codex
 * @key csp_xmlyfm
 */
...
function mapAlbum(item) {
  ...
  return {
    ...
    ext: { gid: GID.ALBUM_TRACKS, id: id, albumId: id, type: 'album' }
  }
}
// 单集实体映射
...
async function loadAlbumTracks(albumId, page = 1) {
  ...
  while (currentPage <= maxPage) {
    ...
    if (page ? true : pageTracks.length < 100) break;
    ...
    try {
      ...
      if (list.length > 0) {
        return list.map(item => {
          ...
          return {
            id: artistId, name: artistName, title: artistName, cover: artistCover, artwork: artworkCover, avatar: artistCover,
...
async function getSongs(ext) {
  ...
  const id = parsed?.id || parsed?.albumId || parsed?.artistId
  const text = parsed?.text
  const type = parsed?.type

  const gidValue = `${gid ?? ''}`
  let list = []
  if (gidValue == GID.ALBUM_TRACKS || type === 'album' || id) {
