/*!
 * @name xmlyfm4 (原版对照最小修复版)
 * @description 喜马拉雅FM（基于原v1.6.4修复专辑列表+内页加载 + [限免]标注）
 * @version v1.7.1-mod
 * @author codex + Grok 最小修改
 * @key csp_xmlyfm
 */
...
// 原版 loadRecommendedAlbums / loadAlbumsByKeyword 基本不变，增加更宽松提取
  ...
  while (currentPage <= maxPage) {
    ...
    for (const url of urls) {
      try {
        ...
        const extra = { Referer: referer, 'User-Agent': url.includes('mobile') ? MOBILE_UA : UA };
        const data = await fetchJson(url, extra);
        const list = firstArray(data?.data?.tracks, data?.data?.list, data?.tracks, data?.data?.trackList);
        if (list.length > 0) {
          pageTracks = list;
          break;
        }
...
// 其他原版函数（loadArtistTracks、loadTracksByKeyword、loadArtistsByKeyword）保持不变
...
async function getAlbums(ext) {
  ...
  return jsonify({ list: list.map(item => mapAlbum(item)) })
}
