/*!
 * @name xmlyfm4 (限免专辑加载+播放最小修复版)
 * @description 喜马拉雅FM（仅优化限免专辑内列表加载 + 单曲播放链接，基于原v1.6.4）
 * @version v1.7.2-mod
...
// 核心过滤：将所有限免、付费、VIP全部视为收费项拦截（你原版）
...
async function getSongs(ext) {
  ...
  if (gidValue == GID.ALBUM_TRACKS) {
    if (type === 'artist') {
      ...
      list = await loadAlbumTracks(id, page)   // 使用优化后的限免加载
    }
  }
...
async function getSongInfo(ext) {
  ...
  const urls = [
    { url: `https://www.ximalaya.com/mobile-playpage/track/v3/baseInfo/${Date.now()}?device=web&trackId=${trackId}`, ua: UA },
