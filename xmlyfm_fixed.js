...
if (!s.startsWith('http')) {
return '[https://imagev2.xmcdn.com/](https://imagev2.xmcdn.com/)' + s.replace(/^//, '')
}
...
```
  const info = typeof data === 'string'
    ? JSON.parse(data)
    : data

  const d = (
    info?.data && info?.data?.trackInfo
  )
    ? info.data.trackInfo
    : (
      info?.data ||
