/*!
 * @name xmlyfm3
 * @description 喜马拉雅FM (隐藏VIP和付费内容 显示限免)
 * @version v1.2
 * @author codex
 * @key csp_xmlyfm
 */
...
function isPaidItem(item) {
  ...
  const isLimitFree =
    ...
    item.isVipFree === 1 ||
    item.vipFreeType === 1 ||
    (item.free_end_time && new Date(item.free_end_time) > new Date());
...
function mapAlbum(item) {
  ...
  return {
    ...
    artist: {
      ...
      name: artistName,
      title: artistName,
      cover: toHttps(item?.avatar ?? ''),
      avatar: toHttps(item?.avatar ?? ''),
    },
...
function mapTrack(item) {
  ...
  return {
    ...
    ext: { source: XM_SOURCE, trackId: id, title: showName, singer: artistName }
  }
}
  ...
  for (const url of urls) {
    try {
      ...
      const isLimitFree =
        ...
        info.isVip === 1 || info.vipFreeType === 1 ||
