/*!
 * @name QQnianxin
 * @description QQnianxin聚合音乐 (全平台并发直连版)
 * @version v2.5
 * @author kobe (Modified)
 * @key csp_QQnianxin
 */

const UPDATE_URL = "https://raw.gitcode.com/Crystim/mfp/raw/main/QQ_%E5%BF%B5%E5%BF%83.js";
const API_BASE_URL = "https://music.nxinxz.com";
...
function formatAlbumItem(_) {
  return {
    ...
    artwork:
      _.albumPic ||
      `https://y.gtimg.cn/music/photo_new/T002R800x800M000${_.albumMID || _.album_mid
      }.jpg`,
...
async function getMediaSource(musicItem, quality) {
  try {
    ...
    if (!apiLevel) {
      console.error('不支持的音质:', quality);
      throw new Error(`不支持的音质: ${quality}`);
    }
    
    const songId = musicItem.songmid || musicItem.id;
    if (!songId) {
      console.error('找不到歌曲ID', musicItem);
      throw new Error('找不到歌曲ID');
    }
    
    const url = `${API_BASE_URL}${API_PATHS.tx}?id=${encodeURIComponent(songId)}&level=${encodeURIComponent(apiLevel)}&type=mp3`;
    
    console.log('生成的音乐URL:', url);
